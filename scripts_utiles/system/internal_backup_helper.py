...
def find_pg_tool(tool_name):
    # Si está en el PATH, shutil.which lo encuentra (funciona en Linux/Mac y Windows configurado)
    tool_path = shutil.which(tool_name)
    if tool_path:
        return tool_path

    # Si es Windows y no está en el PATH, buscar en directorios estándar de instalación
    if os.name == 'nt':
        program_files = os.environ.get('ProgramFiles', 'C:\\Program Files')
        pg_dir = Path(program_files) / 'PostgreSQL'
        if pg_dir.exists():
            # Buscar carpetas de versión numérica ordenadas de mayor a menor (ej: 16, 15, 14...)
            for version_dir in sorted(pg_dir.glob('[0-9]*'), reverse=True):
...
def run():
    ...
    if target == "LIST_AND_RESTORE":
        ...
        try:
            ...
            proc = subprocess.Popen([psql_bin, '-U', db_conf['USER'], '-h', db_conf['HOST'], db_conf['NAME']], 
        ...
        try:
            ...
            cmd = [pg_dump_bin, '-U', db_conf['USER'], '-h', db_conf['HOST'], db_conf['NAME']]
