...
from apps.gestionDeVentasYFacturacion.cu13_gestionar_estado_de_pedido.services.pedido_service import PedidoService
                ...
                with schema_context(tenant.schema_name):
                    try:
                        ...
                        pedidos = Pedido.objects.filter(
                            carrito__cliente__correo=email
                        ).select_related(
                            'carrito__cliente'
                        ).prefetch_related(
                            'carrito__items__producto__categoria'
                        )
                        for p in pedidos:
                        ...
                        print(f"[WARN] Error en tenant {tenant.schema_name}: {str(e_tenant)}")
            
            return Response(all_pedidos)
        except Exception as e:
            print("[ERROR] ERROR EN BUSQUEDA GLOBAL:")
